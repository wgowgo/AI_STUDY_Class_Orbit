...
def _neis_result_ok(payload: dict[str, Any]) -> None:
    ...
    raise HTTPException(status_code=502, detail=fNEIS: {msg} ({code}))
...
async def public_summary(
    ...
    cache_key = fsummary|{atpt}|{school}
...
async def search_schools(
    ...
    cache_key = fschools|{query}|{atpt_code or ''}|{size}
    cached = _neis_cache.get(cache_key)
    if cached is not None:
        return cached
    ...
    if res.status_code != 200:
        ...
        raise HTTPException(
            status_code=502,
            detail=fNEIS schoolInfo 조회 실패 (status={res.status_code}): {snippet},
        )
...
async def school_timetable(
    ...
    if res.status_code != 200:
        ...
        raise HTTPException(
            status_code=502,
            detail=fNEIS 시간표 조회 실패 (status={res.status_code}): {snippet},
        )
...
async def ai_chat(req: ChatRequest, _auth: None = Depends(verify_internal_api_key)):
    ...
    if res.status_code >= 400:
        raise HTTPException(status_code=502, detail=fOpenAI 호출 실패: {res.text[:200]})
    data = res.json()
...
async def stt_transcribe(
    ...
    if res.status_code >= 400:
        ...
        raise HTTPException(status_code=502, detail=fSTT 실패 (status={res.status_code}): {snippet})
...
def pathway_recommend(req: PathwayRequest):
    ...
    return {
        ...
        summary: (
            f최우선 과목은 {weaknesses[0].subject}입니다. 
...
def warning_check(req: WarningCheckRequest):
    ...
    if trend < -8:
        triggers.append(f최근 성적 {abs(trend):.0f}점 하락 추세)
    if req.attendanceRate < 85:
        triggers.append(f출결률 {req.attendanceRate:.0f}% (기준 85% 미만))
    if req.submissionRate < 80:
        triggers.append(f과제 제출률 {req.submissionRate:.0f}% 저조)
    if req.engagementScore < 55:
        triggers.append(수업 참여도 낮음)
    if score_variance > 12:
        triggers.append(f점수 편차 {score_variance:.1f} 높음 (학습 불안정))
    ...
    return {
        ...
        summary: (
            f위험 수준 [{level.upper()}] – 위험 지수 {risk_score:.0f}/100. 
            + (f주요 원인: {triggers[0]} if triggers else 안정적인 학습 상태입니다.)
        ),
    }
...
def equity_index(req: EquityIndexRequest):
    ...
    return {
        ...
        summary: (
            ...
            f전국 평균 대비 성취도 {'+' if national_gap_vs_avg >= 0 else ''}{national_gap_vs_avg}점.
        ),
    }
...
async def nlq_query(req: NlqRequest, _auth: None = Depends(verify_internal_api_key)):
    ...
    if not api_key:
        ...
        return {
            ...
            keyEntities: {},
            answer: (
                fOPENAI_API_KEY가 설정되지 않아 시뮬레이션으로 응답합니다.\n
                f질문 도메인: {sample_domain}. 
    ...
    if res.status_code >= 400:
        raise HTTPException(status_code=502, detail=fNLQ OpenAI 호출 실패: {res.text[:200]})
